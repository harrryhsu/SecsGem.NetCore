...
namespace SecsGem.NetCore.Test.Test
{
    public class AlarmTest : SecsGemTestBase
    {
        ...
        {
            await _client.Function.ToggleAlarm(1, false);
            Assert.That(_alarm.Enabled, Is.False);

            await _client.Function.ToggleAlarm(1, true);
        ...
        {
            var alarms = await _client.Function.GetAlarmDefinitions();
            ...
            alarms = await _client.Function.GetAlarmDefinitions(new uint[] { 5 });
            Assert.That(alarms.Count(), Is.EqualTo(1));
            fa = alarms.First();
        ...
        {
            var alarms = await _client.Function.GetEnabledAlarmDefinitions();
            Assert.That(alarms.Count(), Is.EqualTo(1));

            await _client.Function.ToggleAlarm(1, false);
            alarms = await _client.Function.GetEnabledAlarmDefinitions();
