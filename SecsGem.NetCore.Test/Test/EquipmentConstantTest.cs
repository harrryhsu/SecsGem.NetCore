...
namespace SecsGem.NetCore.Test.Test
{
    public class EquipmentConstantTest : SecsGemTestBase
    {
        ...
        {
            var vs = await _client.Function.S2F29EquipmentConstantNamelistRequest();
            var values = await _client.Function.S2F13EquipmentConstantRequest(new uint[] { EC.Id });
        ...
        {
            var ack = await _client.Function.S2F15NewEquipmentConstantSend(new List<EquipmentConstant> { new() { Id = EC.Id, Value = -5 } });
            Assert.That(ack, Is.EqualTo(SECS_RESPONSE.EAC.OneOrMoreValueOutOfRange));

            ack = await _client.Function.S2F15NewEquipmentConstantSend(new List<EquipmentConstant> { new() { Id = EC.Id, Value = 205 } });
            Assert.That(ack, Is.EqualTo(SECS_RESPONSE.EAC.OneOrMoreValueOutOfRange));

            ack = await _client.Function.S2F15NewEquipmentConstantSend(new List<EquipmentConstant> { new() { Id = EC.Id + 1, Value = 10 } });
            Assert.That(ack, Is.EqualTo(SECS_RESPONSE.EAC.OneOrMoreConstantDoNotExist));

            ack = await _client.Function.S2F15NewEquipmentConstantSend(new List<EquipmentConstant> { new() { Id = EC.Id, Value = 50 } });
